...
public class Painter extends JPanel implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if (game.isAccepted()) {
            if (game.isYourTurn() && !game.isUnableToCommunicateWithOpponent() && !game.isWon() && !game.isEnemyWon()) {
                int x = e.getX() / game.getLengthOfSpace();
                int y = e.getY() / game.getLengthOfSpace();
                y *= 3;
                int position = x + y;

                if (game.getSpaces()[position] == null) {
                    if (!game.isCircle()) game.getSpaces()[position] = "X";
                    else game.getSpaces()[position] = "O";
